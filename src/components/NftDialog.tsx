...
export function NftDialog({ currency, options, owned, onBuy, onClose }: { currency: NftCurrency; options: NftDefinition[]; owned: string[]; onBuy: (names: string[]) => void; onClose: () => void }) {
  ...
  return <div className="trade-dialog-overlay" role="presentation" onClick={(e) => e.target === e.currentTarget && onClose()}>
    <div className="trade-dialog" role="dialog" aria-modal="true" aria-label="Buy NFT">
      ...
      <header className="trade-dialog-header"><h2 className="trade-dialog-title">Buy NFT</h2><p className="trade-dialog-sub">{currency} collection · fixed prices</p></header>
      <div className={`nft-picker nft-picker-${currency}`} aria-label={`${currency} NFTs`}>
        {options.map((item) => {
          ...
          return <label className={`nft-option${isOwned ? ' owned' : ''}`} key={item.name}>
            ...
            <span className="nft-option-copy"><strong>{item.name}</strong><small>{item.price.toLocaleString()} {currency}</small></span>
          </label>
        })}
      </div>
